...
PostLocationDirective.$inject = ['$http', 'Leaflet', 'Geocoding', 'Maps', '_', 'Notify', '$window', '$rootScope'];
function PostLocationDirective($http, L, Geocoding, Maps, _, Notify, $window, $rootScope) {
    return {
        restrict: 'E',
        replace: true,
        scope: {
        ...
        template: require('./report-location.html'),
    ...
    function PostLocationLink($scope, element, attrs) {
        ...
        function activate() {
            Maps.createMapIndex(element[0].querySelector('.map'))
            .then(function (data) {
                map = data;
        ...
        function onMapClick(e) {
            ...
            $rootScope.reportLocate = {lat: lat, lon: lon};
            //-----------------------------------------------------//
        }
